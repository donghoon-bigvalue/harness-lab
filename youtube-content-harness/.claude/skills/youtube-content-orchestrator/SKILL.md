...
하네스 루트: `youtube-content-harness/`

| 용도 | 경로 |
|------|------|
| 에이전트 정의 | `youtube-content-harness/.claude/agents/{name}.md` |
| 스킬 | `youtube-content-harness/.claude/skills/{name}/SKILL.md` |
| 중간 산출물 | `youtube-content-harness/_workspace/` |
| 최종 산출물 | `youtube-content-harness/outputs/{YYYYMMDD}-{slug}/content-package.md` |

## 팀 구성

리더는 이 스킬을 실행하는 메인 세션이며, `youtube-supervisor` 역할을 수행한다. 시작 시 `youtube-content-harness/.claude/agents/youtube-supervisor.md`를 Read하여 감독자 원칙을 적용한다.

| 팀원 | 정의 파일 | 사용 스킬 | 출력 |
|------|----------|----------|------|
| `trend-researcher` | `.claude/agents/trend-researcher.md` | `youtube-trend-research` | `01_trend_research.md`, `01_angle_candidates.md` |
| `script-writer` | `.claude/agents/script-writer.md` | `youtube-script-writing` | `02_script.md` |
| `seo-optimizer` | `.claude/agents/seo-optimizer.md` | `youtube-seo-optimization` | `03_seo.md` |
| `thumbnail-planner` | `.claude/agents/thumbnail-planner.md` | `youtube-thumbnail-concept` | `03_thumbnail.md` |
| `content-reviewer` | `.claude/agents/content-reviewer.md` | `youtube-content-review` | `04_review.md` |

**팀원 생성 방식:** `agent_type`은 `general-purpose`를 사용하고, 프롬프트 첫 지시로 자신의 정의 파일과 스킬 파일을 Read하게 한다. 하네스가 하위 디렉토리에 격리되어 있어 커스텀 에이전트 타입의 자동 인식이 보장되지 않기 때문이다. `model`은 전원 `opus`.

**프롬프트 필수 요소 (전 팀원 공통):** 팀원은 저장소 루트에서 실행되므로, 프롬프트에 다음 4가지가 빠지면 파일을 엉뚱한 위치에 쓴다.
1. 자신의 에이전트 정의 파일 경로 (`youtube-content-harness/.claude/agents/{name}.md`)
2. 사용할 스킬 파일 경로 (`youtube-content-harness/.claude/skills/{skill}/SKILL.md`)
3. **입력 파일의 `youtube-content-harness/_workspace/...` 전체 경로**
4. **출력 파일의 `youtube-content-harness/_workspace/...` 전체 경로**

## 워크플로우

### Phase 0: 컨텍스트 확인

`youtube-content-harness/_workspace/` 존재 여부로 실행 모드를 정한다.

| 상태 | 모드 | 행동 |
|------|------|------|
| 미존재 | **초기 실행** | Phase 1로 진행 |
| 존재 + 사용자가 부분 수정 요청 | **부분 재실행** | Phase 1 건너뛰고 아래 "부분 재실행" 절차 |
| 존재 + 새 주제/새 입력 | **새 실행** | `_workspace/`를 `_workspace_{YYYYMMDD_HHMMSS}/`로 이동 후 Phase 1 |
...
2. `youtube-content-harness/_workspace/` 생성
3. 입력을 `_workspace/00_input.md`에 기록 (사용자 요청 원문 포함)
...
1. 팀 생성 (1인):
   ...
   TeamCreate(
     ...
     members: [
       { name: "trend-researcher", agent_type: "general-purpose", model: "opus",
         prompt: "youtube-content-harness/.claude/agents/trend-researcher.md 와
                  youtube-content-harness/.claude/skills/youtube-trend-research/SKILL.md 를 Read하고
                  그 역할·절차를 그대로 수행하라. 주제: {주제}. 타깃: {타깃}.
                  산출물: youtube-content-harness/_workspace/01_trend_research.md,
                          youtube-content-harness/_workspace/01_angle_candidates.md" }
     ]
   )
   ```
...
5. 선택되지 않은 앵글은 `_workspace/01_angle_backlog.md`로 보존한다
...
1. content-reviewer 소집, `_workspace/` 전체를 입력으로 지정
...
1. `_workspace/` 산출물을 하나의 발행 가능한 문서로 통합:
   `youtube-content-harness/outputs/{YYYYMMDD}-{slug}/content-package.md`
...
4. `_workspace/`는 **삭제하지 않는다** (사후 검증·부분 재실행의 입력)
...
| content-reviewer 실패 | 감독자가 `youtube-content-review` 스킬의 경계면 5종을 직접 수행 |
...
2. Phase 1 — 주제/타깃/포맷 확정, `_workspace/` 생성
