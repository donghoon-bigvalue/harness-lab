...
|---------|---------------|------|--------|
| style-guardian | `webtoon-studio:style-guardian` | `webtoon-studio:style-consistency` | `01_style_bible.md`, `07_consistency_report.md` |
| story-writer | `webtoon-studio:story-writer` | `webtoon-studio:story-writing` | `02_story_beatsheet.md` |
| character-designer | `webtoon-studio:character-designer` | `webtoon-studio:character-prompting` | `03_character_sheet.json`, `03_character_prompts.md` |
| panel-planner | `webtoon-studio:panel-planner` | `webtoon-studio:panel-layout` | `04_panel_layout.json`, `09_conti.html` |
| dialogue-editor | `webtoon-studio:dialogue-editor` | `webtoon-studio:dialogue-editing` | `05_dialogue_script.json` |

**모든 Agent 호출에 `model: "opus"`를 명시한다.**

에이전트 정의와 스킬은 플러그인이 제공한다. 위 `subagent_type`으로 띄우면 역할·원칙·통신 프로토콜이 시스템 프롬프트로 이미 주입된 상태로 기동하므로, **정의 파일을 Read시키는 지시는 넣지 않는다.** 경로로 참조할 것은 산출물뿐이다.

모든 에이전트 프롬프트에 공통으로 넣을 것 — 에이전트 정의에는 역할만 있고 이번 회차의 경로는 없다:

```
1. 작업 디렉토리: {WORKDIR}
2. 산출물 경로: {WORKDIR}/_workspace/{파일명}
3. 완료 시 반환 메시지는 3줄 이내 요약 + 산출물 경로만. 본문을 반환에 담지 마라.
```
...
`webtoon/episodes/` 와 활성 `webtoon/_workspace/`를 확인해 실행 모드를 결정한다.

| 상황 | 모드 | 행동 |
|------|------|------|
| `webtoon/_workspace/` 없음 | **초기 실행** | Phase 1로 |
| `webtoon/_workspace/` 있음 + 부분 수정 요청 ("대사만", "컷 다시") | **부분 재실행** | 해당 Phase만. 앞 산출물은 재사용하고 덮어쓰지 않는다 |
| `webtoon/_workspace/` 있음 + 새 회차 입력 | **새 실행** | `webtoon/_workspace/`를 `webtoon/_workspace_{YYYYMMDD_HHMMSS}/`로 이동 후 Phase 1 |
| `webtoon/_workspace/` 있음 + 리뷰만 재요청 | **리뷰 재실행** | Phase 4~5만, 라운드 번호 +1 |
...
3. `{WORKDIR}/_workspace/` 생성. `{WORKDIR}`은 하네스를 실행 중인 프로젝트 루트 기준 `webtoon`
4. 입력을 `webtoon/_workspace/00_input.md`에 저장
...
Agent(
  name: "style-guardian",
  subagent_type: "webtoon-studio:style-guardian",
  model: "opus",
  run_in_background: false,
  prompt: "<공통 프롬프트> + 00_input.md와 (있다면) webtoon/series-bible.md를 읽고
           webtoon/_workspace/01_style_bible.md를 작성하라. 모든 규칙에 SB-* ID와 검증 방법을 붙이고,
...
```
Agent(name: "story-writer", subagent_type: "webtoon-studio:story-writer", model: "opus",
      run_in_background: true,
      prompt: "<공통> + 01_style_bible.md, 00_input.md를 읽고 webtoon/_workspace/02_story_beatsheet.md 작성")
```
...
Agent(name: "character-designer", ..., run_in_background: true,
      prompt: "<공통> + 01, 02를 읽고 webtoon/_workspace/03_character_sheet.json + 03_character_prompts.md 작성")
Agent(name: "panel-planner", ..., run_in_background: true,
      prompt: "<공통> + 01, 02를 읽고 webtoon/_workspace/04_panel_layout.json 작성.
               말풍선은 슬롯만 선언하고 텍스트는 쓰지 마라.
               작성 후 render_conti.py로 webtoon/_workspace/09_conti.html을 렌더해 리듬을 자가 검수하라.")
```
...
Agent(name: "dialogue-editor", ..., run_in_background: true,
      prompt: "<공통> + 01,02,03,04를 읽고 webtoon/_workspace/05_dialogue_script.json 작성.
...
1. style-guardian에게 5개 리뷰 종합을 요청 → `webtoon/_workspace/07_consistency_report.md`
2. 리포트의 판정에 따라 분기:
   - `PASS` → Phase 6
   - `REVISE` → 개정 라운드 진행
   - `BLOCKED` → 사용자에게 보고하고 결정을 받는다
3. 개정 라운드: 각 제작 에이전트에게 **자기 앞 개정 지시만** SendMessage로 전달. 수정본은 `webtoon/_workspace/08_r{N}_{artifact}`에 저장하고 원본은 남긴다
...
3. `webtoon/episodes/{회차번호 3자리}-{슬러그}/`에 패키지를 조립한다:
...
4. `webtoon/series-bible.md`에 신규 규칙과 확정된 결정 사항을 병합한다. 이 병합을 건너뛰면 다음 회차가 같은 논쟁을 반복한다

### Phase 7: 정리 및 피드백

1. 살아 있는 에이전트를 종료한다 (`TaskStop`)
2. `webtoon/_workspace/`는 **삭제하지 않는다** — 사후 검증과 감사 추적에 쓴다
...
2. Phase 0 → `webtoon/_workspace/` 없음 → 초기 실행
